...
- `src/app.rs`：应用胶水层。拥有 `Workspace`，运行事件循环，处理输入，触发渲染，关闭 PTY。
...
- `src/input.rs`：把 `crossterm` 的 key event 转成 ScrollMux 动作，或者转成字节转发给当前 PTY。

## 依赖的分工

- `crossterm`：控制宿主终端，包括 raw mode、alternate screen、输入事件、光标移动、颜色和属性输出。
...
5. 主线程用 `crossterm::event::poll()` 轮询输入，默认每 16ms 醒一次。
6. 输入先进入 `input::map()`：
   - 如果是 ScrollMux 快捷键，变成 `Action`，例如切换 focus、创建 pane、移动 viewport。
   - 如果是普通按键，编码成字节写入当前 focused pane 的 PTY。
...
输入路径相反：普通按键被编码成字节，通过 `Pane::write_input()` 写入 PTY writer。
...
1. Alt 组合键优先解释为 ScrollMux 命令，例如：
   - `Alt-h` / `Alt-l`：切换 focus。
   - `Alt-n`：新建 pane。
   - `Alt-w`：关闭当前 pane。
   - `Alt-[` / `Alt-]`：水平移动 viewport。
   - `Alt-=` / `Alt--`：调整当前 pane 宽度。
2. 其他 key event 转成常见终端字节序列，例如：
   - 普通字符转 UTF-8。
   - Enter 转 `\r`。
   - Backspace 转 `0x7f`。
   - 方向键转 xterm 风格 escape sequence。

所以 ScrollMux 的输入处理不是“更新文本框状态”，而是“拦截自己的快捷键；其他尽量像真实终端一样透传给 PTY”。
...
4. 最后读 `src/app.rs` 和 `src/input.rs`，把事件循环和快捷键串起来。
